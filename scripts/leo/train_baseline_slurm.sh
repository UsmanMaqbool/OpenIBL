...
if [ "$#" -lt 5 ]; then
     echo "Arguments error: <METHOD (netvlad|graphvlad> <DATASET (pitts|tokyo)> <DATASET> <SCALE> "
    echo "Try"
    echo "./scripts/train_baseline_slurm.sh netvlad triplet vgg16 pitts 30k triplet"
    echo "or"
    echo "./scripts/train_baseline_slurm.sh graphvlad triplet vgg16 pitts 30k triplet" 
    exit 1
fi
