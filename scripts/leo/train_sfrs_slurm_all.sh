...
## To RUN
# sbatch --j graphvlad-v7 ./scripts/train_sfrs_slurm_all.sh graphvlad pitts
####################################################################################################
...
LOSS="sare_joint"
DATE=$(date '+%d-%b') 
FILES="/home/m.maqboolbhutta/usman_ws/models/openibl/0822-s-2/${ARCH}-${METHOD}-sfrs-${LOSS}-${DATASET}${SCALE}-lr${LR}-tuple${GPUS}-${DATE}"
...
python -u examples/netvlad_img_sfrs.py --launcher slurm --tcp-port ${PORT} \
  ...
  --width 640 --height 480 --tuple-size 1 -j 4 --test-batch-size ${CACHEBS} \
...
do
  ...
  $PYTHON -m torch.distributed.launch --nproc_per_node=$GPUS --master_port=$PORT --use_env \
   examples/test_pitts_tokyo.py --launcher pytorch \
    -a ${ARCH} --test-batch-size ${CACHEBS} -j 4 \
