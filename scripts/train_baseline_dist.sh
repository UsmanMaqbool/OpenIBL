...
examples/netvlad_img.py --launcher pytorch --tcp-port ${PORT} \
  ...
  --eval-step 1 --epochs 20 --step-size 5 --cache-size 1000 \
  --logs-dir=/media/leo/2C737A9872F69ECF/why-so-deepv2-data/pittsburgh/netvlad-run/${DATASET}${SCALE}-${ARCH}/${LAYERS}-${LOSS}-lr${LR}-tuple${GPUS}
