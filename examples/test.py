...
def get_model(args):
    ...
    if args.vlad:
        ...
        if(args.method=='netvlad'):
            ...
            model = models.create('graphvlad', base_model, pool_layer, segmentation_model, NB=6)
    else:
        model = base_model
...
def main_worker(args):
    ...
    recall = evaluator.evaluate(test_loader_q, sorted(list(set(dataset.q_test) | set(dataset.db_test))),
    ...
    if (args.rank==0):     
        if args.dataset == 'pitts':
            if args.scale == '30k':
                recallpitts30k = recall
                recallpitts250k = [0, 0, 0]  # Placeholder values
                recalltokyo = [0, 0, 0]  # Placeholder values
            elif args.scale == '250k':
                recallpitts250k = recall
                recallpitts30k = [0, 0, 0]  # Placeholder values
                recalltokyo = [0, 0, 0]  # Placeholder values
        elif args.dataset == 'tokyo':
            recalltokyo = recall
            recallpitts30k = [0, 0, 0]  # Placeholder values
            recallpitts250k = [0, 0, 0]  # Placeholder values
        recalls = np.concatenate((recallpitts250k, recallpitts30k, recalltokyo))    
